...
const HomePage=(props)=> {
  const {settype,handelProductDetail}=useContext(ProductContext);
    ...
    const [HuawieProduct, setHuawieProduct] = useState([]);
      ...
      useEffect(()=>{
 Axios.post("http://localhost:8088/product/GetproductbyType",{type:"سامسونگ"}).then(res=>{
...
{ParchamProduct.length>0?(
           <div><ProductShow product={AppleProduct} NameOfBranch='گوشی های پرچم دار'/></div> ):(<div>.....myskeleton</div>)
}
...
{XiomieProduct.length>0?(
          ...
          <ElsticSlide itemms={AppleProduct} time='14000'/>
            </div>):(<div>.....myskeleton</div>)
}
{HuawieProduct.length>0?(
          ...
          <ElsticSlide itemms={AppleProduct} time='10000'/>
            </div>):(<div>.....myskeleton</div>)
}
